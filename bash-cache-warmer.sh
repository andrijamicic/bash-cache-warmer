...
warm_mobile() {
    ...
    curl -sL -A "$USER_AGENT_MOBILE" http://$1/sitemap_index.xml | egrep -o "http(s?)://$1[^ \"\'()\<>]+" | while read line; do
        if [[ $line == *.xml ]]
        then
            newURL=$line
...
warm_desktop() {
    ...
    curl -sL -A "$USER_AGENT_DESKTOP" http://$1/sitemap_index.xml | egrep -o "http(s?)://$1[^ \"\'()\<>]+" | while read line; do
        if [[ $line == *.xml ]]
        then
            newURL=$line
